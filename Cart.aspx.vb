...
Partial Class Cart
    ...
    Sub checkingout()
        ...
        If loginStatus Then
            ...
            If addressID <> "" Then
                Try
                    returnDate = CDate(cldReturn.Text)
                    lblReturnReminder.Visible = False
                    If returnDate > Date.Now.Date.AddDays(30) Then
                        ...
                        End If
                    Else
                        lblReturnReminder.Visible = True
                        lblReturnReminder.Text = "Are you serious to borrow item(s) so short?"
                    End If
                Catch ex As Exception
                    cldReturn.Text = "Choose a date"
                End Try
            End If
        End If
    End Sub
    ...
    Protected Sub btnAddAddress_Click(sender As Object, e As EventArgs) Handles btnAddAddress.Click
        ...
        If Name.IndexOf(" ") > 1 Then
            ...
            Else
                ...
                lblAddressValid.Text = "Name, Email, Street1, City, State and Zip Codes are required"
            End If
        Else
    ...
    Sub SendMail()
        ...
        mail.Subject = gv_Email.Rows(0).Cells(2).Text & "'s Order"
        mail.Body = "Dear " & gv_Email.Rows(0).Cells(2).Text & ",<br/> Thanks for Submitting an Order! You order information is shown below.<br/> We will process your order as soon as possible! <br/><br/>Best Regards,<br/>Dan Stovall<br/>Perpetual Amity Furnishing and Travel, INC.<br/>Phone:815-761-3815<br/>Email:wsa.dbs@gmail.com<br/> <p><a href='http://omisapps.niu.edu/Tank/paft/Account/Manage.aspx'>Please login your account to pay deposit for your order.</a></p>"
        mail.Body += GetGridviewData(gv_Email)

...
Partial Class Account_Payment
    ...
    Protected Sub btnCheckPayment_Click(sender As Object, e As EventArgs) Handles btnCheckPayment.Click
        ...
        If btnCheckPayment.Text = "Check My Payment Summary" Then
            ...
            mystr = "Dear " & gv_Email.Rows(0).Cells(2).Text & ",<br/> Thanks for planning a payment for your order! Here is a summary of your paying plan:<br/>"
            mystr += "<table><tr><td>Your total deposit for this order is</td><td>$" & lblTotal.Text & ".00</td></tr><tr><td>The delivery fee you are willing to pay is</td><td>$" & ddlDeliveryFee.SelectedValue & ".00</td></tr><tr><td>You are willing to make a donation of </td><td>$" & ddlDonation.SelectedValue & ".00</td></tr><tr><td>The total amount you are going to pay for this order is </td><td>$" & CStr(CInt(lblTotal.Text) + CInt(ddlDeliveryFee.SelectedValue) + CInt(ddlDonation.SelectedValue)) & ".00</td></tr></table><br/>Note: Your deposit is refundable when you return our furniture!<br/>"
            mystr += "Here is your order summary:<br/>"
    ...
    Sub SendMail()
        ...
        Dim mystr As String = ""
        mail.Subject = gv_Email.Rows(0).Cells(2).Text & "'s Paying plan"
        mystr = "Dear " & gv_Email.Rows(0).Cells(2).Text & ",<br/> Thanks for planning a payment for your order! Here is a summary of your paying plan:<br/>"
        mystr += "<table><tr><td>Your total deposit for this order is</td><td>$" & lblTotal.Text & ".00</td></tr><tr><td>The delivery fee you are willing to pay is</td><td>$" & ddlDeliveryFee.SelectedValue & ".00</td></tr><tr><td>You are willing make a donation of </td><td>$" & ddlDonation.SelectedValue & ".00</td></tr><tr><td>The total amount you are going to pay for this order is </td><td>$" & CStr(CInt(lblTotal.Text) + CInt(ddlDeliveryFee.SelectedValue) + CInt(ddlDonation.SelectedValue)) & ".00</td></tr></table><br/>Note: Your deposit is refundable when you return our furniture!<br/><br/>"
        mystr += "Here is your order summary:<br/>"
        mystr += GetGridviewData(gv_Email)
        mystr += "<br/>We are glad to help you. We will process your order as soon as possible!<br/><br/>Best Regards,<br/>Dan Stovall<br/>Perpetual Amity Furnishing and Travel, INC.<br/>Phone:815-761-3815<br/>Email:wsa.dbs@gmail.com<br/>"
        mail.Body = mystr
